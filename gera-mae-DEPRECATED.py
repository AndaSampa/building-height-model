...
for index, quadricula in gdf_articulacao.iterrows():
    ...
    if os.path.isfile(f'./resultados/MAE-{quadricula.nome}.gpkg'):
        print(f'MAE-{quadricula.nome}.gpkg já existe!')
    else:
        xmin, ymin, xmax, ymax = quadricula.geometry.bounds
        pipeline[0]['bounds'] = f'([{xmin}, {xmax}], [{ymin}, {ymax}])'
        pipeline[4]['filename'] = f'./resultados/MAE-{quadricula.nome}.gpkg'
        pipeline[5]['filename'] = f'./resultados/MAE-{quadricula.nome}.tif'
    ...
    if not os.path.isfile(f'./resultados/MAE-{quadricula.nome}.tif.zip'):
        
        tif_zip = zipfile.ZipFile(f'./resultados/MAE-{quadricula.nome}.tif.zip', 'w')
        tif_zip.write(f'./resultados/MAE-{quadricula.nome}.tif', compress_type=zipfile.ZIP_DEFLATED)
