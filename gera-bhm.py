...
for index, quadricula in gdf_articulacao.iterrows():
    ...
    if os.path.isfile(f'./resultados/BHM-{quadricula.nome}.gpkg'):
        ...
        pipeline[4]['filename'] = f'./resultados/BHM-{quadricula.nome}.tif'
    ...
    if not os.path.isfile(f'./resultados/BHM-{quadricula.nome}.tif.zip'):
        
        tif_zip = zipfile.ZipFile(f'./resultados/BHM-{quadricula.nome}.tif.zip', 'w')
        tif_zip.write(f'./resultados/BHM-{quadricula.nome}.tif', compress_type=zipfile.ZIP_DEFLATED)
